...
class Spltr:
    ...
    `Spltr.process_x|y` : converts loaded Input/Target data into PyTorch 
        ...
        iv) use selected rows/columns from Input/Target data sources 
    ...
    def process_x(self, preview=False, tensor_dtype=None, tensor_device=None, csv_sep=',', header='infer', nrows=None, usecols=None):
        '''
        ...
        header = [for Pandas DataFrame only] int, list of int, default 'infer'
    ...
    def process_y(self, preview=False, tensor_dtype=None, tensor_device=None, csv_sep=',', header='infer', nrows=None, usecols=None):
        '''
        ...
        # Sanity check to see if we have Input and Target subsets of the same length in the end.        
...
#         assert len(self.x) == len(self.y), "Attention: [X] and [y] are of different length."
    ...
    def split_data(self, splits=0.5, batch_size=1, perm_seed = False, loader_shuffle=False):        
        ...
        splits : int, list or tuple with 2 numerical values, default = 0.5
            Identifies split ratio which shall be under 1.0 (represents 100%)
            i) int --> splits data into 2 dataloaders where Train = int #1, 
                Test = 1.0 - int #1
        ...
        if type(splits) is float:
            ...
            self.xy_train = DataLoader(self.xy_train, batch_size=batch_size, shuffle=loader_shuffle)
            self.xy_test = DataLoader(self.xy_test, batch_size=batch_size, shuffle=loader_shuffle)             
            ...
            if len(splits) == 1:
                ...
                self.xy_train = DataLoader(self.xy_train, batch_size=batch_size, shuffle=loader_shuffle)
                self.xy_test = DataLoader(self.xy_test, batch_size=batch_size, shuffle=loader_shuffle)
                self.xy_val = DataLoader(self.xy_val, batch_size=batch_size, shuffle=loader_shuffle)                
